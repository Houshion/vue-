<template>
    <div id="chooseType" class="pd-15">
        <div class="boxShadow type flex flexStart pd-15 radius">
            <van-button class="typeItem mg-t-10 cccc radius5" v-for="(item,index) in typeData" :key="index" @click="change('tp',item.id)">{{item.name}}</van-button>
            <van-button class="modeItem mg-t-10 cccc radius5" v-for="(item,index) in modeData" :key="index" @click="change('md',item.id)" :color="item.status?'':'cmain'">{{item.name}}</van-button>
        </div>
        <div class="countTime boxShadow mg-t-15 radius" v-if="complete">
            ...
            <div class="bottom">
                <img src="@/program/xhamy/img/wave.png" alt="">
            </div>
        </div>
    </div>
</template>
