<template>
    <div id="place" class="pd-15">
        <o-refresh ref="reFresh" @onLoad="onLoad" @refresh="refresh" v-if="list.length>0">
            <o-list :title="item.name" v-for="(item,index) in list" :key="index" :url="{
                ...
                <div class="title tal" slot="title">
                    ...
                    <div class="c999">设备编号：{{item.macno}}</div>
                </div>
                <div slot="right" class="h100 box price">
                    <span class="font24" :class="item.status==1?'free':item.status==(2||3||5)?'using':'stop'">{{item.status==1?"空闲":item.status==(2||3||5)?"使用中":item.status==4?"故障":"离线"}}</span>
                </div>
            </o-list>
        </o-refresh>
        ...
        init() {
            ...
            this.axios.post("/wxsite/agent/api", this.form).then(res => {
                ...
                if (res.data.length < 20) {
                    setTimeout(() => {
                        _this.$refs.reFresh.finished = true
                        _this.$refs.reFresh.loading = false
                    }, 1000);
                }
            })
        },
