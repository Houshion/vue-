...
export default {
  ...
  data() {
    return {
      logo: require("@/program/xhamy/img/logo.jpg"),
      boxMsg: {
        username: {
          placeholder: "请输入登录账号",
          // label: "账号",
          icon: require('@/program/xhamy/img/icon_current.png'),
          value: null
        },
        password: {
          placeholder: "请输入登录密码",
          // label: "密码",
          icon: require('@/program/xhamy/img/icon_lock.png'),
          value: null
        },
        // sms: {
    ...
    login(val) {
      ...
      this.axios.post("/wxsite/agent/api", this.form).then(res => {
        ...
        if (res.code == 1) {
          ...
          _this.base.setItem("agentType", res.data.type)
          setTimeout(() => {
