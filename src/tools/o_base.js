...
export default {
   ...
   * @param {string} explode 分隔符，不传默认为?
   * @param {string} url 链接
   */
  ...
  /**
   * 亮屏熄屏操作
   * @param {function} open 亮屏回调动作
   * @param {function} close 熄屏回调动作
   */
  listenView(open, close) {
  ...
  changeUrl(url) {
    let param = ''
    if (this.getUrlParms("code")) {
      if (url.split("#")[1].split("?")[1]) {
        param = url.split("#")[1].split("?")[1] + "&"
      }
      return url.split("?")[0] + "#/" + param + "code=" + this.getUrlParms("code")
    }
  },
  isWX_Allipay() {
